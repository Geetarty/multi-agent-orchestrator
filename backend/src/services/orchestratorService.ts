...
export class OrchestratorService {
  ...
  private getRetryStrategy(retryCount: number): string {
    if (retryCount <= 0) return "Initial attempt: implement the full solution directly from the owner spec.";
    if (retryCount === 1) return "Retry 1: pass the prior error back directly and fix only what failed.";
  ...
  private getRelevantSpecSection(ownerRequirements: OwnerRequirements, focus?: string): string {
    if (!focus) return ownerRequirements.specificationDetails;
